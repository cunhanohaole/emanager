CREATE  TABLE IF NOT EXISTS `emanager`.`address_group` (
...
CREATE  TABLE IF NOT EXISTS `emanager`.`email_address` (
	`id` INT(11) NOT NULL AUTO_INCREMENT ,
	...
	CONSTRAINT `fk_em_email_address_em_group`
		...
		REFERENCES `emanager`.`address_group` (`id` )
			ON DELETE NO ACTION
			ON UPDATE NO ACTION
);

CREATE  TABLE IF NOT EXISTS `emanager`.`email` (
	`id` INT(11) NOT NULL AUTO_INCREMENT ,
...
CREATE  TABLE IF NOT EXISTS `emanager`.`attachment` (
	`id` INT(11) NOT NULL AUTO_INCREMENT ,
